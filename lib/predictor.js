...
var means = [ 0, 0, 0, 0, 0, 0, 1, 513.5, 6, 30.4, 0.5 ],	// Means of features
	ranges = [ 1, 1, 1, 1, 1, 1, 2, 1027, 12, 60.8, 1 ],	// Ranges of features
...
function parse(readings) {
	...
	for (var i = 0, n = readings.length; i < n; i++) {
		...
		for (j = 1; j < 6; j++) {
			features[i].vector.pdateSingleValue(j, parseFloat((features[i].vector - means[j]) / ranges[j]);
		}
	}
