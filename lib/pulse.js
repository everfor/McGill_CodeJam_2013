...
function populateEntries(entries, endCallback, type, data) {
	...
	if(completed == 5) {
		endCallback(interpolate(entries));
	}
}
...
function interpolate(entries) {
	...
	for(var i = start ; i < tidyEntries.length - 4 + start; i += 4) {
		var next = i + 4;
	...
	var end = tidyEntries.length - 4 + start; //Last index with data everywhere
