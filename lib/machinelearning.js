...
function predict(feature, weights) {
	...
	if (feature.vector[5] > 0) {
		prediction = feature.innerProductWith(weights) * (feature.innerProductWith(weights) > 14500 ? 1.6 / 1.7 : 1.7 / 1.6);
	} else {
		prediction = feature.innerProductWith(weights);
	}

	if (feature.vector[3] >= (100 / 720)) {
		prediction -= 1500;
	}
