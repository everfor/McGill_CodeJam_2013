	...
	public void movePossible(Pacman pacman, int[][] board, Graphics g) {
		...
		if(scatter){
			if(goLeftGhost && (exitLeft || exitUp || exitDown)) {
				if(exitUp){
					distanceUp = distance(4, 3, x, y - 1);
				}
				else {
					distanceUp = 9999;
				}
				if(exitLeft){
					distanceLeft = distance(4, 3, x - 1, y);
				}
				else {
					distanceLeft = 9999;
				}
				if(exitDown){
					distanceDown = distance(4, 3, x, y + 1);
				}
				else {
					distanceDown = 9999;
				}
			...
			if(goRightGhost && (exitRight || exitUp || exitDown)) {
				...
				if(exitRight){
					distanceRight = distance(4, 3, x + 1, y);
				}
				else {
					distanceRight = 9999;
				}
			...
			if(goLeftGhost && (exitLeft || exitUp || exitDown)) {
				changeOffset();
				if(exitUp){
					distanceUp = distance(pacman.getX()+ xOffset, pacman.getY()+ yOffset, x , y - 1 );
				}
				else {
					distanceUp = 9999;
				}
				if(exitLeft){
					distanceLeft = distance(pacman.getX()+ xOffset, pacman.getY()+ yOffset, x - 1 , y);
				}
				else {
					distanceLeft = 9999;
				}
				if(exitDown){
					distanceDown = distance(pacman.getX()+ xOffset, pacman.getY()+ yOffset, x , y + 1 );
				}
				else {
					distanceDown = 9999;
				}
			...
			if(goRightGhost && (exitRight || exitUp || exitDown)) {
				...
				if(exitRight){
					distanceRight = distance(pacman.getX()+ xOffset, pacman.getY()+ yOffset, x + 1 , y );
				}
				else {
					distanceRight = 9999;
				}
