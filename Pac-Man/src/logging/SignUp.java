...
public class SignUp extends SignUpGUI{
	...
	/**
	 * Checks if Username contains invalid characters. Allowed characters are:
	 * lowercase letters, uppercase letters, numbers, underscores, dashes and
	 * fullstops
	 * 
	 * @param usernameInput
	 ...
	 * process and flags which errors occur. This is done through the use of
	 * global boolean variables, one for every error. The errors that it checks
	 * for are the following: 1)If the Username already exists 2)If the Password
	 * and re-enter password fields are equal 3)If the Password length is not
	 * between the given 8-45 characters 4)If the Username length is not between
	 * the given 6-16 characters 5)If the Username contains special characters
	...
	/**
	 * This method uses the error booleans in order to generate a string that
	 * includes all the errors encountered during the signUp Process this string
	 * is then used to tell the player what needs to be changed in order to
	...
	/**
	 * This method checks the player inputted data for errors using
	 * signupError() and creates a personalized error message if errors do
	 * exist. These errors are shown in a pop-up window. If no errors are found
	 * this method attempts to add a new entry into the database. If it does so
	 * successfully the player is notified by a pop-up window that he has been
	 * added to the database. Otherwise if an error occurs while inputting the
	 * data into the database the player is also notified via a pop-up window
	 * that the process was unsuccessful
	 * 
	 * @param newUsername
	 *            the username inputted by the player
	 * @param newPassword
	 *            the password inputted by the player
	 * @param securityQuestion
	 *            the security question inputted by the player
	 * @param securityAnswer
	 *            the answer to the security question inputted by the player
